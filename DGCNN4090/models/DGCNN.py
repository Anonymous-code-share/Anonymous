...
class DGCNN_main(nn.Module):
    def __init__(self, num_classes=40, embedding=512, k=32):
        super(DGCNN_main, self).__init__()
        self.k = k
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128*2, 256, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv1d(512, embedding, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.linear1 = nn.Linear(embedding*2, 512, bias=False)
...
def DGCNN(num_classes=40, **kwargs) -> DGCNN_main:
    return DGCNN_main(num_classes=num_classes)
