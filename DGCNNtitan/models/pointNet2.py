...
def sin(data):
    ...
    k = k.to(device)
    if n == 3:
        data[:,1,:] = data[:,1,:] + k * alpha1.to(device) * torch.sin(beta1.to(device) * data[:,1,:])
        data[:,2,:] = data[:,2,:] + k * alpha2.to(device) * torch.sin(beta2.to(device) * data[:,2,:])
        data[:,0,:] = data[:,0,:] + k * alpha3.to(device) * torch.sin(beta3.to(device) * data[:,0,:])
    elif c == 3:
        data[:, :, 1] = data[:, :, 1] + k * alpha1.to(device) * torch.sin(beta1.to(device) * data[:, :, 1])
        data[:, :, 2] = data[:, :, 2] + k * alpha2.to(device) * torch.sin(beta2.to(device) * data[:, :, 2])
        data[:, :, 0] = data[:, :, 0] + k * alpha3.to(device) * torch.sin(beta3.to(device) * data[:, :, 0])
...
class pointNet2_msg(nn.Module):
    def __init__(self, num_classes=40, k=24):
...
class pointNet2_ssg(nn.Module):
    def __init__(self, num_classes=40, k=32):
...
def pointNet2msg(num_classes=40, **kwargs) -> pointNet2_msg:
    return pointNet2_msg(num_classes=num_classes)


def pointNet2ssg(num_classes=40, **kwargs) -> pointNet2_ssg:
    return pointNet2_ssg(num_classes=num_classes)
