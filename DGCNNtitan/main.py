...
Usage:
nohup python main.py --model DGCNN --msg test --seed 3214 >> test.txt 2>&1 &
"""
import argparse
import os
import logging
...
def parse_args():
    ...
    parser.add_argument('--model', default='DGCNN', help='model name [default: pointnet_cls]')
    ...
    parser.add_argument('--seed', type=int, default=3214, help='random seed')
    parser.add_argument('--workers', default=4, type=int, help='workers')
    """SMT"""
    parser.add_argument('--rand_center_num', type=int, default=0, help='message after checkpoint')
    parser.add_argument('--alphamin', type=float, default=-0.5, help='min')
    parser.add_argument('--alphamax', type=float, default=0.5, help='max')
    parser.add_argument('--f', type=float, default=5, help='f')
    parser.add_argument('--sample', type=str, default='random', help='type')
    parser.add_argument('--isCat', type=bool, default=True, help='Point Number')
    parser.add_argument('--shuffle', type=bool, default=True, help='learning rate in training')
...
def main():
    ...
    train_loader = DataLoader(ScanObjectNN(partition='train', num_points=args.num_points, ver="easy"), num_workers=args.workers,
                                batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ScanObjectNN(partition='test', num_points=args.num_points, ver="easy"), num_workers=args.workers,
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_out = train(net, train_loader, optimizer, criterion, device,SMT1)  # {"loss", "acc", "acc_avg", "time"}
...
def train(net, trainloader, optimizer, criterion, device,SMT):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        data,label = SMT.sin_transform(data,label)
        #print(data)
        data = data.data.numpy()
        key = torch.randint(1, 8, size=[1, ])
        if key == 1:
            data = provider.shift_point_cloud(data, shift_range=0.4)
        if key == 2:
            data = provider.rotate_point_cloud(data)
        if key == 3:
            data = provider.random_point_walk(data)
        if key == 4:
            data = provider.shift_point_cloud(data, shift_range=0.3)
            data = provider.rotate_point_cloud(data)
        if key == 5:
            data = provider.shift_point_cloud(data, shift_range=0.3)
            data = provider.random_point_walk(data)
        if key == 6:
            ...
            data = provider.random_point_walk(data)
        if key == 7:
            data = provider.shift_point_cloud(data, shift_range=0.2)
            data = provider.rotate_point_cloud(data)
            data = provider.random_point_walk(data)
        if key == 8:
            data = data
