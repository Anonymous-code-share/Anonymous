...
Usage:
nohup python main2.py --model DGCNN --msg sinpoint_3090 >> sinpoint_3090.txt 2>&1 &
"""
import argparse
import os
import logging
...
def parse_args():
    ...
    parser.add_argument('--seed', type=int,default=7546, help='random seed')
    parser.add_argument('--workers', default=4, type=int, help='workers')

    """SinPoint"""
    parser.add_argument('--rand_center_num', type=int, default=0, help='k=0: Global and k>0: Local')
    parser.add_argument('--w', type=float, default=2.5, help='[-w,w]')
    parser.add_argument('--A', type=float, default=0.6, help='[-A,A]')
    parser.add_argument('--sample', type=str, default='RPS', help='RPS: random point sampling, FPS: farther point sampling ')
    parser.add_argument('--isCat', type=bool, default=True, help='isCat')
...
def main():
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_out = train(net, train_loader, optimizer, criterion, device,SinDA)  # {"loss", "acc", "acc_avg", "time"}
...
def train(net, trainloader, optimizer, criterion, device,SinPoint):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        data, label = SinPoint.Sin(data, label)
        #print(data)
